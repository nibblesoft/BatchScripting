...
@echo off
	...
	title Release Named Clipper v5.6
:menu
	color 07
	cls
	echo.
	...
	echo.
	echo                     ===============================================
	echo                         Go to subtitles folder and copy the path
	echo                         and paste it in here to allow the scripts to
	echo                                      run correctly
	echo                     ================================================
	rem this command will show us current location in the scripts
	echo.
	echo Current Location = %__cd__%
	set "location="
	Rem we can't use ===^> without any symbols back it will return error
	echo Past the location in here and press ENTER
	set /p "location= +===> "
	...
	rem i have used ^(trailing backslash\^)
		...
		REM CASE SENCETIVE SKIPEDIN LINE BELOW
		if /i "!location!\" == "%~dp0" (
			echo.
			echo The folder you just enterd is the same as the scripts^^!
	...
	if %errorlevel% gtr 0 (
		...
		echo make sure you entered the valid path
		echo the scripts will no run until you entered
		echo the valid path
		pause >nul && cls && goto :menu
...
:set
		...
		for /f "tokens=* delims=" %%G in ('dir /b /o "*.srt"') do (
			...
			REM THIS LINE WILL ONLY BR TRUE FIRST TIME THE SCRIPTS RUN
			if !i! equ 1 (
				set "file_name=%%G"
				REM "%PROGRAMFILES%\Winrar\"rar.exe a %%~nG ^^!filename^^!
				echo Archive this file --^> !file_name!
				pause >nul
			)
			...
			if !i! gtr 1 (
				...
				echo.
				echo Comparing the two files below:
				echo ===========================================================
				echo 1^)---^> %%~G
				echo 2^)---^> !file_name!
				echo ===========================================================
