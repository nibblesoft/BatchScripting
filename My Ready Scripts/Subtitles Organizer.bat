...
@Echo off && Cls
	...
	Pushd "!driver!:\" >Nul 2>&1
		If %errorlevel% neq 0 (
			...
			Echo make sure you type the correct driver
			Pause >Nul
			:check
				Echo.
				Echo.
	...
	Echo                      #2) --^> Remove Subtitles from Subs-folder
...
:DAMAGE_SUBTITLES
	...
	for /r "%driver%:\" %%G in ("*.srt") do (
		...
		call :errorlevel
			If !error! gtr 0 (
				...
				REM Echo,
				Echo Damaged Subtitle #!counter! - %%~nG
					If !counter! equ 1 (
	...
	Echo                             Total Damaged Sub-Files [%counter%]
	Echo                             Total UnDamaged Sub-Files [!Damage!]
	Echo                         =================================
	Pause >Nul
	Goto :Menu
